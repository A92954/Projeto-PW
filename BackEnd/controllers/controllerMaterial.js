...
function readConfirmarMaterialUsado(req, res) {
  const id_material = req.params.id_material;
  const query = connect.con.query('SELECT m.nome_material, om.quantidade_usada FROM ocorrencia_material om, material m WHERE m.id_material = ? and om.id_material = m.id_material and om.confirmado_material = 1', id_material,
  function (err, rows, fields) {
    if (!err) {
      if (rows.length == 0) {
        res.status(404).send({ msg: "Data not found"});
