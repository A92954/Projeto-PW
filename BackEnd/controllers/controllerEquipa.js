...
function readMembrosEquipa(req,res) {
  ...
  const query = connect.con.query('SELECT eq.id_equipa,op.username FROM equipa eq,operacional op WHERE eq.id_equipa = op.id_equipa AND eq.id_equipa = ?',id_equipa,
    function(err,rows,fields){
      if (!err) {
        if (rows.length == 0) {
          res.status(404).send({ msg: "data not found" });
