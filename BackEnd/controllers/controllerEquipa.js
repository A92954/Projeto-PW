...
function readEquipaOcorrencia(req, res) {
  ...
  const query = connect.con.query('SELECT eq.* FROM equipa eq, ocorrencia oc WHERE oc.id_ocorrencia = ? and eq.id_equipa = oc.id_equipa', id_ocorrencia,
  function (err, rows, fields) {
    if (!err) {
      if (rows.length == 0) {
        res.status(404).send({ msg: "Data not found" });
